...
const semanticChecks = [
  ...
  ["long if", "iffin true ~~{print(1);} otherwise ~~{print(3);}"],
  ["elsif", "iffin true ~~{print(1);} otherwise iffin true ~~{print(0);} otherwise ~~{print(3);}"],
  ["for over collection", "for i in [2,3,5] ~~{print(1);}"],
  ["for in range", "for i in 1..<10 ~~{print(0);}"],
  ["repeat", "repeat 3 ~~{tag a -= 1; print(a);}"],
  ...
  ["arithmetic", "tag x-=1;print(2*3+5**-3/2-5%8);"],
  ["array length", "print(#[1,2,3]);"],
  ["optional types", "tag x -= no int; x -= some 100;"],
  ["random with array literals, ints", "print(random [1,2,3]);"],
  ["random with array literals, strings", 'print(random ["a", "b"]);'],
  ["random on array variables", "tag a-=[true, false];print(random a);"],
  ["variables", "tag x-=[[[[1]]]]; print(x[0][0][0][0]+2);"],
  ["pseudo recursive struct/ranch", "ranch S -x-x-x-x- z: S?-x-x-x-x- tag x -= S(no S);"],
  ["nested struct/ranch", "ranch T -x-x-x-x- y:int -x-x-x-x- ranch S -x-x-x-x- z: T -x-x-x-x- tag x-=S(T(1)); print(x.z.y);"],
  ["member exp", "ranch S -x-x-x-x- x: int -x-x-x-x- tag y -= S(1);print(y.x);"],
  ["optional member exp", "ranch S -x-x-x-x- x: int -x-x-x-x- tag y -= some S(1);print(y?.x);"],
  ["subscript exp", "tag a-=[1,2];print(a[0]);"],
  ...
  ["task assign", "task f() ~~{} tag g -= f; tag h -= [g, f]; print(h[0]());"],
  ...
  ["outer variable", "tag x-=1; till(false) ~~{print(x);}"],
...
const semanticErrors = [
  ...
  ["non-int decrement", 'tag x-=some[""];x++;', /an integer/],
  ...
  ["assign bad optional type", "tag x-=1;x-=some 2;", /Cannot assign a int\? to a int/],
  ...
  ["non-integer index", "tag a-=[1];print(a[false]);", /Expected an integer/],
  ["no such field", "ranch S -x-x-x-x- -x-x-x-x- tag x-=S(); print(x.y);", /No such field/],
